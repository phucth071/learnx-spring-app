...
public interface AssignmentRepository extends JpaRepository<Assignment, Long> {
    ...
    @Query("SELECT a FROM Assignment a JOIN a.module m JOIN m.course c WHERE c.teacher.email = ?1")
    List<Assignment> findAllByTeacherEmail(String teacherEmail);
    ...
    @Query("SELECT a FROM Assignment a JOIN a.module m JOIN m.course c WHERE c.teacher.id = ?1 AND MONTH(a.endDate) = ?2 AND YEAR(a.endDate) = ?3")
    List<Assignment> findAllByTeacherIdAndEndDateMonthYear(Long teacherId, int month, int year);
